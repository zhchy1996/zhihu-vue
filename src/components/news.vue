  ...
  <div class="news">
    <img :src="img" alt="">
      <div class="my-wrapper">
        <div class="get" v-html="getData"></div>
        <div class="see-answer">
          <span @click="changelongview">{{str.longStr}}长评{{'('+long.length+')'}}</span>
          <span @click="changeshortview">{{str.shortStr}}短评{{'('+short.length+')'}}</span>
        </div>
        <div class="long-answer-view" v-if="longview.now">
          <div class="answer-item" v-for="item in long" :key="item.id" >
         ...
         <div class="short-answer-view" v-if="shortview.now">
          <div class="answer-item" v-for="item in short" :key="item.id" >
            <div class="answer-head">
              <img :src="item.avatar" alt="">
              <span>{{item.author}}</span>
            </div>
...
export default {
  ...
  created:function () {
    ...
    var url = '/api/4/news/' + id;
    var longAnswer = url + '/long-comments';
    var shortAnswer = url + '/short-comments'
    var $self = this;
    this.$http.get(url).then(function (data) {
      console.log(data)
      $self.img= data.body.image;
      $self.getData = data.body.body;
      this.$http.get(longAnswer).then(function (data) {
        this.long = data.body.comments;
      })
      this.$http.get(shortAnswer).then(function (data) {
        this.short = data.body.comments;
      })

    })
  },
  methods:{
    changelongview:function () {
      this.shortview.now = false;
      this.longview.now = !this.longview.now;
    },
    changeshortview:function () {
      this.longview.now = false;
      this.shortview.now = !this.shortview.now;
    }
  },
  computed:{
    str:function () {
      var longStr = ''
      var shortStr = '';
      if(this.shortview.now) {
        shortStr = '隐藏'
      }else{
        shortStr = '显示'
      }
      if(this.longview.now) {
        longStr = '隐藏'
      }else{
        longStr = '显示'
      }
...
.dudu-night .origin-source.with-link:after {
  ...
  .news .headline .img-place-holder {
    height: 10px;
  }
  .news .view-more{
    display:none;
  ...
  .news .long-answer-view,.short-answer-view{
    margin-top:20px;
  ...
  .news .answer-item .answer-head{
    display: flex;
    flex-direction: row;
    align-items: center;
  }
  .news .answer-item .answer-head img {
    width:50px;
    height: 50px;
    flex-grow: 0;
    border-radius: 50%;
  ...
  .news .answer-item .answer-head span{
    color:#666;
    margin-left:30px
  }
  .news .answer-item .answer-body{
    margin-top:10px;
    color:#616466;
  }
</style>
